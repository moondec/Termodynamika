...
## Przemiana 1: Mieszanie Dwóch Strumieni Powietrza

::: {.callout-note title="Zasada" appearance="simple"}
Przy adiabatycznym mieszaniu dwóch strumieni powietrza o różnych stanach (1 i 2), stan wynikowy (M) leży **na odcinku łączącym punkty 1 i 2** na wykresie h-X. Położenie punktu M zależy od stosunku masowego strumieni.
:::

::: {.columns}
::: {.column width="45%"}
**Bilans masy powietrza suchego:**
$$ \dot{m}_{g,M} = \dot{m}_{g,1} + \dot{m}_{g,2} $$

**Bilans wilgoci:**
$$ X_M = \frac{\dot{m}_{g,1} X_1 + \dot{m}_{g,2} X_2}{\dot{m}_{g,1} + \dot{m}_{g,2}} $$

**Bilans entalpii:**
$$ h_M = \frac{\dot{m}_{g,1} h_1 + \dot{m}_{g,2} h_2}{\dot{m}_{g,1} + \dot{m}_{g,2}} $$

::: {.fragment}
**Reguła dźwigni:**
$$ \frac{\overline{1M}}{\overline{M2}} = \frac{\dot{m}_{g,2}}{\dot{m}_{g,1}} $$
:::
:::
::: {.column width="55%"}
```{r}
...
# Punkty stanu
p1 <- list(x = 5, t = 15)   # Zimne powietrze zewnętrzne
p2 <- list(x = 12, t = 35)  # Ciepłe powietrze wewnętrzne
# Proporcja mieszania (60% strumień 1, 40% strumień 2)
r <- 0.6
...
# Wykres bazowy (uproszczony)
ggplot() +
    # Linia mieszania
    geom_segment(aes(x = p1v$x, y = p1v$t, xend = p2v$x, yend = p2v$t),
                 color = "purple", size = 1.5, linetype = "dashed") +
    # Punkty
    geom_point(aes(x = p1v$x, y = p1v$t), color = "blue", size = 5) +
    geom_point(aes(x = p2v$x, y = p2v$t), color = "red", size = 5) +
    geom_point(aes(x = pMv$x, y = pMv$t), color = "green4", size = 6, shape = 18) +
    # Etykiety
    annotate("text", x = p1v$x, y = p1v$t + 3, label = "1 (zewn.)", color = "blue", size = 5, fontface = "bold") +
    annotate("text", x = p2v$x, y = p2v$t + 3, label = "2 (wewn.)", color = "red", size = 5, fontface = "bold") +
    annotate("text", x = pMv$x + 1, y = pMv$t - 2, label = "M (mieszanina)", color = "green4", size = 5, fontface = "bold") +
    # Osie
    scale_x_continuous(limits = c(0, 25), breaks = seq(0, 25, 5)) +
    scale_y_continuous(limits = c(10, 45), breaks = seq(10, 45, 5)) +
    labs(x = "X [g/kg]", y = "t [°C]", title = "Mieszanie adiabatyczne") +
...
## Przemiana 2: Nawilżanie Powietrza Parą Wodną

::: {.callout-note title="Zasada" appearance="simple"}
Przy nawilżaniu **parą nasyconą** ($h_w \approx 2500$ kJ/kg) kierunek przemiany jest niemal **poziomy** (wzdłuż izotermy). Temperatura powietrza prawie się nie zmienia, rośnie tylko zawilżenie $X$.
:::

::: {.columns}
::: {.column width="45%"}
**Bilans masy:**
$$ \dot{m}_{g} X_1 + \dot{m}_p = \dot{m}_{g} X_2 $$
$$ \Delta X = X_2 - X_1 = \frac{\dot{m}_p}{\dot{m}_g} $$

**Bilans entalpii:**
$$ \dot{m}_{g} h_1 + \dot{m}_p h_w = \dot{m}_{g} h_2 $$

::: {.fragment}
**Współczynnik kierunkowy:**
$$ \varepsilon = \frac{\Delta h}{\Delta X} = h_w \approx 2500 \text{ kJ/kg} $$
*(Linia Randa dla pary)*
:::
:::
::: {.column width="55%"}
```{r}
...
# Stan początkowy
p1 <- list(x = 5, t = 25)
# Stan końcowy (nawilżone parą - prawie ta sama temperatura)
p2 <- list(x = 15, t = 25.5) # Lekki wzrost t przez przegrzanie pary
...
ggplot() +
    ...
    # Punkty
    geom_point(aes(x = p1v$x, y = p1v$t), color = "blue", size = 5) +
    geom_point(aes(x = p2v$x, y = p2v$t), color = "green4", size = 5) +
    # Etykiety
    annotate("text", x = p1v$x, y = p1v$t + 3, label = "1 (suche)", color = "blue", size = 5, fontface = "bold") +
    annotate("text", x = p2v$x, y = p2v$t + 3, label = "2 (nawilżone)", color = "green4", size = 5, fontface = "bold") +
    # Informacja o kierunku
    annotate("text", x = 10, y = p1v$t - 5, label = "ε ≈ 2500 kJ/kg\n(prawie izoterma)", 
             color = "gray30", size = 4, fontface = "italic") +
    # Osie
    scale_x_continuous(limits = c(0, 25), breaks = seq(0, 25, 5)) +
    scale_y_continuous(limits = c(15, 40), breaks = seq(15, 40, 5)) +
    labs(x = "X [g/kg]", y = "t [°C]", title = "Nawilżanie parą wodną") +
    theme_minimal(base_size = 16) +
    coord_fixed(ratio = 0.5)
```
:::
...
::: {.callout-note title="Zasada" appearance="simple"}
**Suszenie konwekcyjne** polega na: (A) podgrzaniu powietrza (wzrost $t$, spadek $\varphi$, $X=const$), a następnie (B) przepuszczeniu go przez wilgotny materiał (spadek $t$, wzrost $X$, proces adiabatyczny - wzdłuż izentalpy).
:::

::: {.columns}
::: {.column width="45%"}
**Etap A: Podgrzewanie (nagrzewnica)**
*   $X_A = X_0$ (brak wymiany wilgoci)
*   $t_A > t_0$ (wzrost temperatury)
*   $\varphi_A \ll \varphi_0$ (suche powietrze)

**Etap B: Suszenie (komora suszarnicza)**
*   $h_B \approx h_A$ (izentalpa, adiabata)
*   $X_B > X_A$ (pobranie wilgoci z materiału)
*   $t_B < t_A$ (ochłodzenie przez odparowanie)

::: {.fragment}
**Sprawność procesu** zależy od różnicy $(X_B - X_0)$.
:::
:::
::: {.column width="55%"}
```{r}
...
# Stan 0: Powietrze atmosferyczne
p0 <- list(x = 8, t = 20)
# Stan A: Po nagrzewnicy
pA <- list(x = 8, t = 60)
# Stan B: Po komorze suszarniczej (wzdłuż izentalpy)
# h = const => t spadło, X wzrosło
# Przyjmijmy X_B = 25 g/kg
pB <- list(x = 25, t = 30) # Uproszczenie - dokładne wartości wymagają obliczeń
...
ggplot() +
    # Etap A: Podgrzewanie (pionowo w górę)
    geom_segment(aes(x = p0v$x, y = p0v$t, xend = pAv$x, yend = pAv$t),
                 arrow = arrow(length = unit(0.3, "cm")),
                 color = "red", size = 1.5) +
    # Etap B: Suszenie (wzdłuż izentalpy - w dół w prawo)
    geom_segment(aes(x = pAv$x, y = pAv$t, xend = pBv$x, yend = pBv$t),
                 arrow = arrow(length = unit(0.3, "cm")),
                 color = "blue", size = 1.5) +
    # Punkty
    geom_point(aes(x = p0v$x, y = p0v$t), color = "gray40", size = 5) +
    geom_point(aes(x = pAv$x, y = pAv$t), color = "red", size = 5) +
    geom_point(aes(x = pBv$x, y = pBv$t), color = "blue", size = 5) +
    # Etykiety
    annotate("text", x = p0v$x - 2, y = p0v$t, label = "0 (atmosf.)", color = "gray40", size = 4, fontface = "bold", hjust = 1) +
    annotate("text", x = pAv$x - 2, y = pAv$t, label = "A (po nagrzew.)", color = "red", size = 4, fontface = "bold", hjust = 1) +
    annotate("text", x = pBv$x + 1, y = pBv$t, label = "B (wylot suszarni)", color = "blue", size = 4, fontface = "bold", hjust = 0) +
    # Opisy etapów
    annotate("text", x = p0v$x + 1, y = (p0v$t + pAv$t)/2, label = "A: Ogrzewanie\n(X = const)", color = "red", size = 3.5, hjust = 0) +
    annotate("text", x = (pAv$x + pBv$x)/2, y = pAv$t - 5, label = "B: Suszenie\n(h ≈ const)", color = "blue", size = 3.5) +
    # Osie
    scale_x_continuous(limits = c(0, 35), breaks = seq(0, 35, 5)) +
    scale_y_continuous(limits = c(15, 70), breaks = seq(15, 70, 5)) +
    labs(x = "X [g/kg]", y = "t [°C]", title = "Cykl suszarniczy") +
    theme_minimal(base_size = 16) +
    coord_fixed(ratio = 0.5)
```
:::
...
::: {.callout-note title="Zasada" appearance="simple"}
Przy nawilżaniu **wodą** ($h_w \approx 0$ kJ/kg dla wody w temp. otoczenia) kierunek przemiany biegnie **wzdłuż izentalpy** (linia Randa $\varepsilon \approx 0$). Temperatura powietrza **spada**, wilgotność rośnie. Jest to podstawa działania **chłodni kominowych** i **klimatyzatorów ewaporacyjnych**.
:::

::: {.columns}
::: {.column width="45%"}
**Bilans entalpii:**
$$ \dot{m}_{g} h_1 + \dot{m}_w c_w t_w \approx \dot{m}_{g} h_2 $$

Ponieważ $c_w t_w \ll r_0$:
$$ h_1 \approx h_2 $$

::: {.fragment}
**Współczynnik kierunkowy:**
$$ \varepsilon = \frac{\Delta h}{\Delta X} \approx 0 $$
*(Proces wzdłuż izentalpy)*
:::

::: {.fragment}
**Temperatura graniczna:** Temperatura mokrego termometru $t_m$ (najniższa możliwa do osiągnięcia).
:::
:::
::: {.column width="55%"}
```{r}
...
# Stan początkowy (gorące suche powietrze)
p1 <- list(x = 8, t = 35)
# Stan końcowy (ochłodzone nawilżone - wzdłuż izentalpy)
# h = const, X rośnie, t spada
p2 <- list(x = 20, t = 22) # Uproszczenie
...
ggplot() +
    # Strzałka przemiany (wzdłuż izentalpy)
    geom_segment(aes(x = p1v$x, y = p1v$t, xend = p2v$x, yend = p2v$t),
                 arrow = arrow(length = unit(0.4, "cm")),
                 color = "steelblue", size = 1.5) +
    # Punkty
    geom_point(aes(x = p1v$x, y = p1v$t), color = "red", size = 5) +
    geom_point(aes(x = p2v$x, y = p2v$t), color = "blue", size = 5) +
    # Etykiety
    annotate("text", x = p1v$x, y = p1v$t + 3, label = "1 (gorące suche)", color = "red", size = 5, fontface = "bold") +
    annotate("text", x = p2v$x, y = p2v$t - 3, label = "2 (chłodne wilgotne)", color = "blue", size = 5, fontface = "bold") +
    # Informacja o kierunku
    annotate("text", x = 14, y = p1v$t + 5, label = "ε ≈ 0 kJ/kg\n(wzdłuż izentalpy)", 
             color = "gray30", size = 4, fontface = "italic") +
    # Osie
    scale_x_continuous(limits = c(0, 30), breaks = seq(0, 30, 5)) +
    scale_y_continuous(limits = c(15, 50), breaks = seq(15, 50, 5)) +
    labs(x = "X [g/kg]", y = "t [°C]", title = "Nawilżanie adiabatyczne (wodą)") +
    theme_minimal(base_size = 16) +
    coord_fixed(ratio = 0.5)
```
:::
