...
generate_mollier_data <- function() {
    ...
    df_iso_list <- lapply(t_iso_levels, function(temp) {
        ...
        # Współczynnik wzniosu (dobrany eksperymentalnie dla estetyki)
        dry_slope <- 0.08
        
        seg1 <- data.frame(
            x_g = x_vals_s1,
            t = temp + dry_slope * x_vals_s1,
        ...
        # Punkt startowy mgły musi pasować do końca odcinka suchego (z uwzględnieniem wzniosu)
        t_sat_visual <- temp + dry_slope * x_sat_g
        
        # Obliczamy 'efektywną' entalpię, która przechodzi przez ten wizualny punkt nasycenia
        # h_sat_vis = cp * t_vis + x_sat * (r0 + cp_v * t_vis)
        # Używamy tej entalpii, aby kontynuować linię w dół w obszarze mgły
        h_sat_visual <- CONSTANTS$cp_dry * t_sat_visual + x_sat_kg * (CONSTANTS$r_0 + CONSTANTS$cp_vap * t_sat_visual)
        
        # Zakres x we mgle
        x_fog_g <- seq(x_sat_g, x_max_g, length.out = 10)
        x_fog_kg <- x_fog_g / 1000
        
        # Temperatura we mgle (dla h_vis = const)
        t_fog <- (h_sat_visual - x_fog_kg * CONSTANTS$r_0) / (CONSTANTS$cp_dry + x_fog_kg * CONSTANTS$cp_vap)
    ...
    fan_list <- lapply(eps_values, function(eps) {
        ...
        # Równanie prostej: t - pole_y = slope * (x_kg - pole_x)
        # t = pole_y + slope * x_kg
        ...
        t_at_xmax <- pole_y + slope * x_end_kg

        if (t_at_xmax <= t_max && t_at_xmax >= t_min) {
            # Trafia w prawą krawędź
            x1 <- x_max_g
            y1 <- t_at_xmax
            side <- "right"
        } else {
            # Trafia w górę lub dół
            if (slope > 0) {
                # Góra (t = 55)
                # 55 = 25 + slope * x -> x = (30)/slope
                x_top_kg <- (t_max - pole_y) / slope
                x1 <- x_top_kg * 1000
                y1 <- t_max
                side <- "top"
            } else {
                # Dół (t = -10)
                # -10 = 25 + slope * x -> x = (-35)/slope
                x_bot_kg <- (t_min - pole_y) / slope
                # Dodać zabezpieczenie przed x ujemnym (dla bardzo stromych)
                # Ale tu x rośnie w prawo, więc ok dla slope < 0
                x1 <- x_bot_kg * 1000
                y1 <- t_min
                side <- "bottom"
            }
        ...
        # Ograniczanie do zakresu wykresu (na wypadek błędów numerycznych)
        if (x1 > x_max_g) x1 <- x_max_g
        # Dla bardzo pionowych linii (skrajnych)
        if (x1 < 0) x1 <- 0 # nie powinno wystąpić dla slope != Inf

        # Obliczamy kąt rotacji dla tekstu, aby biegł wzdłuż linii
        # Uwzględniamy coord_fixed(ratio = 0.5), co "rozciąga" oś X względem Y
        # Tangens kąta wizualnego = ratio * (dy/dx)
        vis_slope <- 0.5 * ((y1 - pole_y) / (x1 - pole_x))
