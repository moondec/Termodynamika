...
generate_mollier_data <- function() {
    ...
    # Zagęszczamy wartości, szczególnie dla pary (2500)
    eps_values <- sort(unique(c(
        seq(0, 1500, 500),          # Woda (<2000)
        seq(2000, 3200, 200),       # Para (zakres roboczy) - gęsto
        2500,                       # Dokładna wartość dla r_0
        seq(3500, 5000, 500)        # Wysokie entalpie
    )))
    ...
    fan_list <- lapply(eps_values, function(eps) {
        ...
        # 1. Sprawdzamy prawą ścianę (x = x_max)
        x_end_kg <- x_max_g / 1000
        ...
        if (t_at_xmax <= t_max && t_at_xmax >= t_min) {
            ...
            # 2. Jeśli nie prawa, to góra lub dół
            if (slope > 0) {
                 # Góra (t = t_max)
                 x1 <- (t_max / slope) * 1000
                 y1 <- t_max
        ...
        # Obliczamy kąt rotacji dla tekstu, aby biegł wzdłuż linii
        # Uwzględniamy coord_fixed(ratio = 0.5), co "rozciąga" oś X względem Y
        # Tangens kąta wizualnego = 2 * (dy/dx)
        vis_slope <- 2 * (y1 / x1)
        angle <- atan(vis_slope) * 180 / pi
        
        # Pozycjonowanie "wewnątrz" - cofamy się wzdłuż linii o ok. 8-10%
        # Dzięki temu etykieta będzie na końcu linii, ale wewnątrz wykresu
        factor <- 0.92
        lab_x <- x1 * factor
        lab_y <- y1 * factor
        
        data.frame(x0=0, y0=0, x1=x1, y1=y1, eps=eps, side=side, lab_x=lab_x, lab_y=lab_y, angle=angle)
    })
...
p <- ggplot() +
    ...
    # Opis jednostki skali Randa (dopasowany kąt ok. 45 stopni też by wyglądał dobrze, ale zostawmy poziomo)
    annotate("text", x = 40, y = 53, label = "[\u0394h/\u0394x kJ/kg]", color = "gray40", size=3.5, fontface="italic", hjust=1) +
