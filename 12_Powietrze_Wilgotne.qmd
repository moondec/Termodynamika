    ...
    fan_list <- lapply(eps_values, function(eps) {
        ...
        if (t_at_xmax <= t_max && t_at_xmax >= t_min) {
            ...
            side <- "right"
             ...
             if (slope > 0) {
                 # Góra (t = 55) (tu t oznacza t_phys)
                 # Ale uwaga: t_max na wykresie to t_vis? Nie, granice wykresu t_min/t_max są w t_phys czy t_vis?
                 # Skoro rysujemy t_vis, to granica wykresu Y=55 jest wizualna.
                 # t_vis = t_phys + slope_vis * x
                 # 55 = (25 + slope_phys * x) + slope_vis * x
                 # 55 - 25 = x * (slope_phys + slope_vis)
                 # x = 30 / (slope_phys + slope_vis)
                 
                 effective_slope <- slope + vis_slope_val*1000 # vis_slope_val jest na gram, slope na kg -> uwaga na jednostki!
                 # vis_slope_val = 0.08 [deg/g] = 80 [deg/kg]
                 # slope [deg/kg]
                 
                 x_top_kg <- (t_max - pole_y) / (slope + vis_slope_val * 1000)
                 x1 <- x_top_kg * 1000
                 y1_phys <- pole_y + slope * x_top_kg
                 side <- "top"
             } else {
                 # Dół (t = -10)
                 # Analogicznie:
                 x_bot_kg <- (t_min - pole_y) / (slope + vis_slope_val * 1000) # Tu mianownik może być różny
                 
                 # Wzór ogólny na przecięcie z poziomą linią Y=Y_limit w układzie wizualnym:
                 # Y_limit = (pole_y + slope*x) + vis_skew*x
                 # Y_limit - pole_y = x * (slope + vis_skew)
                 
                 denom <- slope + vis_slope_val * 1000
                 x1 <- (t_min - pole_y) / denom * 1000
                 y1_phys <- pole_y + slope * (x1/1000)
                 
                 side <- "bottom"
             }
        }
        
        # Ograniczanie do zakresu wykresu
        if (x1 > x_max_g) {
             x1 <- x_max_g
             y1_phys <- pole_y + slope * (x1/1000)
             side <- "right" # Wymuszenie right jeśli wyszło poza
        }
        if (x1 < 0) x1 <- 0 
        
        # Transformacja wizualna Y
        y1 <- transform_t(y1_phys, x1)
