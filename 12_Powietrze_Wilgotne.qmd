...
generate_mollier_data <- function() {
    ...
    df_iso_list <- lapply(t_iso_levels, function(temp) {
        ...
        # Temperatura we mgle (dla h = const)
        # h = t * (cp_dry + x * cp_vap) + x * r0  => t = (h - x*r0) / (cp_dry + x*cp_vap)
        t_fog <- (h_sat - x_fog_kg * CONSTANTS$r_0) / (CONSTANTS$cp_dry + x_fog_kg * CONSTANTS$cp_vap)
    ...
    # E. Pęk Linii (Wykres tła - skala h_w) - "Biegun" w (0,0)
    # Epsilon = (dh/dx) ~ cp * dt/dx + r0
    # Slope dt/dx_kg = (Epsilon - r0) / cp
    eps_values <- c(0, 400, 800, 1200, 1600, 2000, 2500, 3500, 5000)
    fan_list <- lapply(eps_values, function(eps) {
        slope <- (eps - CONSTANTS$r_0) / CONSTANTS$cp_dry

        # Równanie prostej z (0,0): t = slope * x_kg
        # Szukamy przecięcia z ramką: x in [0, 45 g], t in [-10, 55 deg]

        # Sprawdzamy potencjalne końce
        x_end_g <- x_max_g
        x_end_kg <- x_end_g / 1000
        t_at_xmax <- slope * x_end_kg

        # Czy trafia w prawą ścianę?
        if (t_at_xmax <= t_max && t_at_xmax >= t_min) {
            return(data.frame(x0=0, y0=0, x1=x_end_g, y1=t_at_xmax, eps=eps, side="right"))
        }

        # Jeśli nie prawa, to góra lub dół
        if (slope > 0) {
            # Trafia w górę (t = 55)
            # 55 = slope * x_kg -> x_kg = 55 / slope
            x_top_kg <- t_max / slope
            return(data.frame(x0=0, y0=0, x1=x_top_kg*1000, y1=t_max, eps=eps, side="top"))
        } else {
            # Trafia w dół (t = -10)
            x_bot_kg <- t_min / slope
            return(data.frame(x0=0, y0=0, x1=x_bot_kg*1000, y1=t_min, eps=eps, side="bottom"))
        }
    })
...
p <- ggplot() +
    ...
    geom_segment(
        ...
        color = "gray80", size = 0.5, linetype = "longdash"
    ) +
    geom_text(
        data = data$fan,
        aes(x = x1, y = y1, label = eps, 
            hjust = ifelse(side == "right", 0, 0.5),
            vjust = ifelse(side == "top", 0, ifelse(side == "bottom", 1, 0.5))
        ),
        size = 3.5, color = "gray50", fontface = "bold",
        # Przesunięcie etykiet odrobinę na zewnątrz ramki
        nudge_x = ifelse(data$fan$side == "right", 0.5, 0),
        nudge_y = ifelse(data$fan$side == "top", 0.5, ifelse(data$fan$side == "bottom", -0.5, 0))
    ) +
