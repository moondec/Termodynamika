...
generate_mollier_data <- function() {
    ...
    fan_list <- lapply(eps_values, function(eps) {
        ...
        # Obliczamy pozycje etykiet z marginesem (w jednostkach wykresu)
        lab_x <- x1
        lab_y <- y1
        
        offset_x <- 2  # 2 g/kg w prawo
        offset_y <- 2  # 2 stopnie w pionie
        
        if (side == "right") lab_x <- x1 + offset_x
        if (side == "top")   lab_y <- y1 + offset_y
        if (side == "bottom") lab_y <- y1 - offset_y

        data.frame(x0=0, y0=0, x1=x1, y1=y1, eps=eps, side=side, lab_x=lab_x, lab_y=lab_y)
    })
...
p <- ggplot() +
    ...
    theme(
        # Marginesy: Góra, Prawa, Dół, Lewa (Zwiększone dla etykiet skali Randa)
        plot.margin = margin(t = 1.5, r = 2.0, b = 0.5, l = 0.5, "cm"),
