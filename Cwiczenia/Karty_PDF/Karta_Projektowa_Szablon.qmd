...
txt_mlodszy <- if(is_woman) "Młodsza Inżynierka Energetyk" else "Młodszy Inżynier Energetyk"
```

\begin{center}
\Large\textbf{Indywidualna Karta Projektowa} \\[0.3em]
\normalsize `r if(sa) "\\textcolor{red}{KLUCZ ODPOWIEDZI}" else paste(txt_role, ":", params$student_name)`
\end{center}
...
**Metodyka:** Prawo Daltona: $p_i = y_i \cdot p$. Masa: $m_i = \frac{p_i \cdot V}{R_i \cdot T}$. ($R_{N2}=297, R_{O2}=260$).
...
- $\rho_{rzecz}$ = `r ans(rho_zb, "kg/m^3")`
- $\rho_{norm}$ = `r ans(rho_norm, "kg/m^3")`
...
- Temperatura nasycenia $t_{sat}$ dla ciśnienia `r p$p_kociol` bar wynosi: ............ $^\circ$C.
...
**Metodyka:** Twierdzenie Gouy--Stodoli: $\dot{B}_{str} = T_0 \cdot \Delta\dot{S}_{gen}$, gdzie $T_0 =$ `r p$T0` $^\circ$C (otoczenie).
