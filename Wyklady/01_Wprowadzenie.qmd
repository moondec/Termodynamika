...
Ciśnienie zależy od liczby cząstek $N$, masy pojedynczej cząsteczki $m$ i średniej kwadratu prędkości $\overline{v^2}$.
...
*   Ciśnienie to siła $dF$ wywierana przez cząstki na pole powierzchni $A$:
    $$ p = \frac{dF}{dA} $$
:::
:::
:::::
...
## Modele Gazu: Idealny vs. Rzeczywisty{style="color: #7f2d2dff"}
...
::: {.fragment .callout-note title="Punkt Krytyczny"}
(**$T_k$**): Wraz ze wzrostem temperatury $T$, objętości cieczy ($V_1$) i pary ($V_2$) zbliżają się do siebie. W temperaturze krytycznej $T_k$, $V_1 = V_2 = V_k$.
:::
