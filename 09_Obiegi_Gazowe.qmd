...
ggplot(df_otto, aes(x=v, y=p)) + geom_path(size=2, color="red") +
  annotate("text", x=v_max, y=p1, label="1", vjust=2, size = 6) +
  annotate("text", x=v_min, y=p2, label="2", hjust=2, size = 6) +
  annotate("text", x=v_min, y=p3, label="3", hjust=2, size = 6) +
  annotate("text", x=v_max, y=p4, label="4", vjust=-1, size = 6) +
  annotate("text", x=1.45, y=p3/2, label = "Q['in'] ~ ' (Izochora)'", parse = TRUE, size = 6) +
  theme_bw() + labs(title="Obieg Otto (ZI)", x="v", y="p") +
  ylim(c(-5, p3+5))
...
ggplot(df_diesel, aes(x=v, y=p)) + geom_path(size=2, color="blue") +
  annotate("text", x=v_max, y=p1, label="1", vjust=2, size = 6) +
  annotate("text", x=v_min, y=p2, label="2", hjust=2, size = 6) +
  annotate("text", x=v3, y=p3, label="3", hjust=-1, size = 6) +
  annotate("text", x=v_max, y=p4, label="4", vjust=-1, size = 6) +
  annotate("text", x=4, y=p3, label = "Q['in'] ~ ' (Izobara)'", parse = TRUE, size = 6) +
  theme_bw() + labs(title="Obieg Diesla (ZS)", x="v", y="p") +
  ylim(c(-5, p3+5))
```
...
$$ \eta_{Diesel} = 1 - \frac{1}{\kappa\varepsilon^{\kappa-1}} \left[ \frac{\varphi^\kappa - 1}{\varphi - 1} \right] $$
